...
namespace bulls_and_cows {
    ...
    Board create_board(const GameOptions& game_options)
    {
        ...
        for (unsigned int j = 0; j < game_options.number_of_characters_per_code;
             j++) // pour j allant de 0 à number_of_characters_per_code(5)
        {
            char temp = generate_random_character(
                game_options.minimum_allowed_character,
                game_options.maximum_allowed_character); // générer un caractère entre le
                                                         // minimum/maximum_allowed_character ici définis de A à Z
    ...
    bool validate_attempt(const GameOptions& game_options, const Code& attempt)
    {
        if (attempt.value.size() !=
            game_options.number_of_characters_per_code) // si la taille de ta tentative est différente que
        ...
        for (const char attempt_char : attempt.value)
        {
            if (attempt_char < game_options.minimum_allowed_character ||
                attempt_char > game_options.maximum_allowed_character) // vérifie que les caractère sojn bien compris
    ...
    {
        for (AttemptAndFeedback tempo : board.attempts_and_feedbacks) // récupére un à un les élément du vecteur en les
    ...
    void display_board(std::ostream& output_stream, const GameOptions& game_options, const Board& board)
    {
        ...
        for (auto element : board.attempts_and_feedbacks) // element parcourt le vecteur de tentative attempt and
                                                          // feedbacks qui est composé de bulls and cows
        {
