...
namespace bulls_and_cows {
    ...
    bool validate_attempt(const GameOptions& game_options, const Code& attempt)
    {
        ...
        {
            if (a < game_options.minimum_allowed_character ||
                a > game_options.maximum_allowed_character)
            {
                return false;
            }
        }
   ...
   void display_board(std::ostream& output_stream, const GameOptions& game_options, const Board& board)
    ...
    {
        Code attempt;
        output_stream << "merci de renter un code au nombre de lettres de "
                      << game_options.number_of_characters_per_code << " la lettre la plus petite est "
                      << game_options.minimum_allowed_character << " et la plus grande est "
                      << game_options.maximum_allowed_character << std::endl;
        input_stream >> attempt.value;

        while (!validate_attempt(game_options,attempt))
        {
            output_stream << "votre proposition est invalide, faites en un autre\n"
                          << "merci de renter un code au nombre de lettres de "
                          << game_options.number_of_characters_per_code << " la lettre la plus petite est "
                          << game_options.minimum_allowed_character << " et la plus grande est "
                          << game_options.maximum_allowed_character << std::endl;
            input_stream >> attempt.value;
