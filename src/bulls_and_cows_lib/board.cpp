   ...
   Board create_board(const GameOptions& game_options)
    {
       ...
       {
           Feedback ans;
           for (std::string::size_type i = 0; i < attempt.value.size(); ++i)
           {
               for (std::string::size_type j = 0; j < secret_code.value.size(); ++j)
               {
       ...
       void display_board(std::ostream& output_stream, const GameOptions& game_options, const Board& board)
       {
           ...
           for (unsigned int i = 0U; i < game_options.max_number_of_attempts; i++)    //for all attempts
           {
               ...
               {                                                                                                                //writting played attempts
                  ...
                  if (game_options.max_number_of_attempts - i < 10)
                   {
                       output_stream << "| #" << '0' << game_options.max_number_of_attempts - i << '   ' << '   ' << codeout + "|" << "   " << board.attempts_and_feedbacks[i].feedback.bulls <<
                                            "   " << "|" << "  " << board.attempts_and_feedbacks[i].feedback.cows <<
                                            "   |\n";
                   }
                   ...
                   {
                       output_stream << "| #" << game_options.max_number_of_attempts - i << '   ' << '   ' << codeout + "|" << "   " << board.attempts_and_feedbacks[i].feedback.bulls << "   " << "|" <<
                                            "  " << board.attempts_and_feedbacks[i].feedback.cows << "   |\n";
                   }
               }
           }
