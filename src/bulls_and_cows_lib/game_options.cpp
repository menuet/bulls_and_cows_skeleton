...
namespace bulls_and_cows {
    ...
    bool load_game_options(std::istream& input_file_stream, GameOptions& game_options)
    {
        ...
        while (std::getline(input_file_stream, ligne))
        {
            ...
            std::string condition = ligne.substr(0, pos);
            std::string number = ligne.substr(pos + 1);
            if (number != "")
            {
                if (condition == "max_number_of_attempts")

                    game_options.max_number_of_attempts = std::atoi(number.c_str());

                else if (condition == "number_of_characters_per_code")
                    game_options.number_of_characters_per_code = std::atoi(number.c_str());

                else if (condition == "minimum_allowed_character")

                    game_options.minimum_allowed_character = number[0];

                else if (condition == "maximum_allowed_character")

                    game_options.maximum_allowed_character = number[0];
