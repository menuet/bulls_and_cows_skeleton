...
bool validGuess (string guess){
    ...
    if(guess.length() != 8){
        //if the player inputted a guess that isn't four characters long, it's already wrong, so reject it
        return false;
    }
    for(int j = 0; j < 8; j++){
        //we increment j to know if the guess is four letters long, go through it letter by letter and make sure each letter is a w, W, b, or B
        if(toupper(guess[j]) != 'W' && toupper(guess[j]) != 'B'){
            return false;
            //if any letter of the guess isn't one of those four, the guess is invalid
        }
    }
...
string upperCaseifier(string s){
    // this function is to make every letter of an input string uppercase
...
int main(int){
    ...
    while (choice != "1" && choice != "2" && choice != "3"){
        //if the choice that they select isn't a valid one, then we slap them on the wrist and ask again
    ...
    * Clean version to read than a do-while loop, because were I to use a do-while loop, I'd need another conditional inside to determine whether or not
    * to show an error message. This way, everything involving the user inputting something correct
    */

    //int guesses; // will serve later in our configuration game to choose our level of difficulty the harder the level the fewer guesses the player will get
    if (choice == "1"){
        cout << " Beat the computer!" << endl;
    }
    else if (choice == "2"){
        cout << "Humanity has been wiped out of earth, watch a game between robots!" << endl;
    }
    else{
        cout << "Configure options (soon to come)" << endl;
    ...
    //there's a chance that the player actually has no idea how to play mastermind, so we offer to teach them
    cin >> learn;

    while(learn != "y" && learn != "Y" && learn != "n" && learn != "N"){
        // we have to validate this input, as well as reprompting every time the user gets it wrong
    ...
    if (showTutorial){
        ...
        cout << "You have 12 guesses to try and determine this sequence.(Easy mode)" << endl;
    ...
    do{    //We are using a do-while loop here to contain the game, since I know players will want to play at least once.
        int numBlack = 0;
        int numWhite = 0;
        //Set up counter variables to hold the number of black pegs and white pegs in the solution.
        srand(8);//Also, seed our random number generator using the time.
        ...
        while (k < 12 && !correct){//this while loop is basically a for loop that players can break out of early if they guess the code
            ...
            if(correct){//If they're right, then go ahead and tell them they one
                ...
                cout << "That's not it, sadly! You have " << 12-k << " guesses left!" << endl;
            }
        ...
        if(!correct){//if we ended up breaking out of the while loop due to running out of guesses, instead of by guessing correctly, print the loss statement
            cout << "You're all out of guesses! You loose!" << endl;
            cout << "My code was ";
